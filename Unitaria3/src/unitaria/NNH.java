...
public class NNH {
        ...
        public ReturnHC  Selector_CustomersNNH(Vehicle V, Map<Integer,Customer> Aux){
            ...
            for(int i=1;i<Aux.size();i++){
                ...
                if((Aux.get(i).getDemand()!=0) && (Aux.get(i).getDemand()<V.getCapacity()) && ( V.getTime_service() + (this.distance(V.getX(),Aux.get(i).getX_coord(),V.getY(),Aux.get(i).getY_coord()))< Aux.get(i).getDue_time()) ){
                    double distance=this.distance(V.getX(),Aux.get(i).getX_coord(),V.getY(),Aux.get(i).getY_coord());
                    double wait;
                    if(V.getTime_service() + (this.distance(V.getX(),Aux.get(i).getX_coord(),V.getY(),Aux.get(i).getY_coord())) < Aux.get(i).getReady_time())
                    {
                        wait=Aux.get(i).getReady_time() - (V.getTime_service() + (this.distance(V.getX(),Aux.get(i).getX_coord(),V.getY(),Aux.get(i).getY_coord())));
                    }else{
                        wait=0;
                    }
                    double due_time=Aux.get(i).getDue_time();
                    double pnd = (distance * 0.4) + (wait * 0.3) + (due_time * 0.3);
                    Aux2.add(Aux.get(i));
                    Aux3.add(pnd);
                }
            }
     ...
     public int getNextElement(VRPTWProblem problem) {
        if(problem.customer_complets()){
            ...
            if(problem.getNumberVehicle()!=0){
                ...
                ReturnHC C =Selector_CustomersNNH(V,problem.getUsing_Customer());
